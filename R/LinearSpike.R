...
LinSpike <- function(baseline, peak, period, duty_cycle, peak_pos, trend, duration, resolution) {
  ...
  for (i in 1:(nrow(Osc)*resolution/period)) {
    Osc[round((((i-1)*(period/resolution))+1):(((i-1)*(period/resolution))+active_prim/resolution+1)),2]=
      ((peak-baseline)/active_prim)*Osc[round((((i-1)*(period/resolution))+1):(((i-1)*(period/resolution))+active_prim/resolution+1)),3]+baseline

    Osc[round((((i-1)*(period/resolution))+active_prim/resolution+1):(((i-1)*(period/resolution))+active_tot/resolution+1)),2]=
      ((baseline-peak)/(active_tot-active_prim))*Osc[round((((i-1)*(period/resolution))+active_prim/resolution+1):(((i-1)*(period/resolution))+active_tot/resolution+1)),4]+peak
    peak=peak*trend
    if(peak <= baseline) {
      peak=baseline
    }
  ...
  if ((nrow(Osc)-(((i)*(period/resolution)))) <= (active_prim/resolution)) {
    Osc[round((((i)*(period/resolution))+1):nrow(Osc)),2]=
      ((peak-baseline)/active_prim)*Osc[round((((i)*(period/resolution))+1):nrow(Osc)),3]+baseline
  } else {
    Osc[round((((i)*(period/resolution))+1):(((i)*(period/resolution))+active_prim/resolution)),2]=
      ((peak-baseline)/active_prim)*Osc[round((((i)*(period/resolution))+1):(((i)*(period/resolution))+active_prim/resolution)),3]+baseline

    if ((nrow(Osc)-(((i)*(period/resolution))+active_prim/resolution)) <= ((active_tot-active_prim)/resolution)) {
      Osc[round(floor(((i)*(period/resolution))+active_prim/resolution+1):nrow(Osc)),2]=
        ((baseline-peak)/(active_tot-active_prim))*Osc[round(floor(((i)*(period/resolution))+active_prim/resolution+1):nrow(Osc)),4]+peak
    } else {
      Osc[round((((i)*(period/resolution))+active_prim/resolution+1):(((i)*(period/resolution))+active_tot/resolution)),2]=
        ((baseline-peak)/(active_tot-active_prim))*Osc[round((((i)*(period/resolution))+active_prim/resolution+1):(((i)*(period/resolution))+active_tot/resolution)),4]+peak
    }
  }
