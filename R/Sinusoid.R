...
#' @details Standards:
#' \itemize{
#'  \item{\code{peak} must be larger than \code{baseline} value}
...
#' @param period oscillation period of the oscillator (reciprocal of the frequency)
...
Sinusoid <- function(baseline, peak, period, duty_cycle, trend, duration, resolution) {
  ...
  Osc=matrix(ncol=3,nrow=length(seq(0,duration,resolution)))
  colnames(Osc)=c("time","osc","temp")
  ...
  if ((nrow(Osc)-(((i)*(period/resolution)))) <= (active_tot/resolution)) {
    Osc[round((((i)*(period/resolution))+1):nrow(Osc)),3]=
      seq(0,((nrow(Osc)-((i)*(period/resolution)))*resolution)-resolution,resolution)
  } else {
    Osc[round((((i)*(period/resolution))+1):(((i)*(period/resolution))+(active_tot)/resolution+1)),3]=
      seq(0,active_tot,resolution)
  }

  for (i in 1:(nrow(Osc)*resolution/period)) {
    Osc[round((((i-1)*(period/resolution))+1):(((i-1)*(period/resolution))+active_tot/resolution+1)),2]=
    (peak-baseline)/2*sin((2*pi)/(period*duty_cycle)*Osc[round((((i-1)*(period/resolution))+1):(((i-1)*(period/resolution))+active_tot/resolution+1)),3]+1.5*pi)+baseline+(peak-baseline)/2
  ...
  if ((nrow(Osc)-(((i)*(period/resolution)))) <= (active_tot/resolution)) {
    Osc[round((((i)*(period/resolution))+1):nrow(Osc)),2]=
      (peak-baseline)/2*sin((2*pi)/(period*duty_cycle)*Osc[round((((i)*(period/resolution))+1):nrow(Osc)),3]+1.5*pi)+baseline+(peak-baseline)/2
  } else {
    Osc[round((((i)*(period/resolution))+1):(((i)*(period/resolution))+active_tot/resolution)),2]=
      (peak-baseline)/2*sin((2*pi)/(period*duty_cycle)*Osc[round((((i)*(period/resolution))+1):(((i)*(period/resolution))+active_tot/resolution)),3]+1.5*pi)+baseline+(peak-baseline)/2
  }
